...
Notation CheckFail := (InjR (InjL #())) (only parsing).
Notation Cancel cfstate := (InjR (InjR cfstate)) (only parsing).

Notation Async' e := (InjLV (InjLV e)) (only parsing).
Notation Await' p := (InjLV (InjRV p)) (only parsing).
Notation CheckFail' := (InjRV (InjLV #())) (only parsing).
Notation Cancel' cfstate := (InjRV (InjRV cfstate)) (only parsing).
...
Section implementation.
  ...
  Definition check_fail : val := (λ: <>, do: (CheckFail))%V.
  Definition fiber_cancel : val := (λ: "cf" "state", do: (Cancel ("cf", "state")))%V.
...
Section specification.
  ...
  Class AsyncCompLib := {
    ...
    fiber_cancel_spec (cf state : val) :
      is_cancel cf state -∗
        EWP fiber_cancel cf state <| coop |> {{ i, io_log i }};
  }.
...
Section protocol_coop.
  ...
  Definition CHECKFAIL : iEff Σ :=
    >> (_: val) >> !(CheckFail') {{True}};
    << (_: val) << ?(#()) {{True}} @ OS.
  (* a.d. not sure where δ should be introduced. *)
  Definition CANCEL : iEff Σ :=
    >> (cf state : loc) >> !(Cancel' ((#cf, #state))%V) {{isCancel cf state}};
    << (i : nat) δ << ?(#i) {{io_log_cancelled δ i}} @ OS.
  
  Definition Coop_pre : iEff Σ → iEff Σ := (λ Coop,
    ASYNC_pre Coop <+> AWAIT <+> CHECKFAIL <+> CANCEL
  )%ieff.
  ...
  Lemma upcl_Coop v Φ' :
    iEff_car (upcl OS Coop) v Φ' ⊣⊢
      ...
      iEff_car (upcl OS CHECKFAIL) v Φ' ∨ 
      iEff_car (upcl OS CANCEL) v Φ'.
  Proof.
  ...
  Lemma upcl_CHECKFAIL v Φ' :
    iEff_car (upcl OS CHECKFAIL) v Φ' ≡
      (⌜ v = CheckFail' ⌝ ∗ True ∗ 
        (True -∗ Φ' #()))%I.
  Proof.
(* rewrite /CHECKFAIL. rewrite (upcl_tele' [tele _] [tele _]). by easy. *)
    iSplit.
    - iIntros "Hallows".
  ...
  Lemma upcl_CANCEL v Φ' :
    iEff_car (upcl OS CANCEL) v Φ' ≡ 
      (∃ (cf state : loc), ⌜ v = Cancel' (#cf, #state)%V ⌝ ∗ isCancel cf state ∗
        (∀ (i : nat) δ, io_log_cancelled δ i -∗ Φ' #i) )%I.
  Proof.
    rewrite /CANCEL. rewrite (upcl_tele' [tele _ _] [tele _ _]). by easy.
  Qed.
End protocol_coop.
...
Section verification.
  ...
  Lemma ewp_checkfail:
    ⊢ EWP (check_fail #()) <| Coop |> {{ _, True }}.
  Proof.
    unfold check_fail. ewp_pure_steps.
    iApply ewp_do_os. rewrite upcl_Coop upcl_CHECKFAIL.
    iRight. iRight. iLeft.
    iSplit; first done. iSplit; first done.
    iIntros "_". done.
  Qed.
  ...
  Lemma ewp_fiber_cancel (cf state : loc) :
    isCancel cf state -∗ 
    EWP (fiber_cancel #cf #state) <| Coop |> {{ v, ∃ (δ: gname) (i: nat), ⌜ v = #i ⌝ ∗ io_log_cancelled δ i ∗ isCancel cf state }}.
  Proof.
    iIntros "#Hcancel". 
    rewrite /fiber_cancel. ewp_pure_steps.
    iApply ewp_do_os. rewrite upcl_Coop upcl_CANCEL.
    iRight; iRight; iRight.
    iExists cf, state.
    iSplit; first done.
    iSplit; first done.
    iIntros (i δ) "#Hio".
    iExists δ, i. iSplit; first done.
    by iSplit.
  Qed.
  ...
  Proof.
    ...
    (* Effect branch. *)
    - iIntros (request k). rewrite upcl_Coop upcl_AWAIT upcl_ASYNC upcl_CHECKFAIL upcl_CANCEL.
      idtac.
      ...
      + 
        ...
        * (* fiber is not cancelled. *)
          ...
          iApply ("Hk" $! I).
          iSpecialize ("HcInv" with "HcSt"). iNext.
          by iApply ("IH_handler" with "Hγ Hq HpInv Hmem HcInv Hcmem Hnone").  
        * iApply (ewp_load with "Hcf"). iIntros "!> Hcf !>".
          iDestruct (lookup_promiseInv with "HpInv Hmem") as "[HpInv HpSt]".
          ewp_pure_steps. 
      ...
      iAssert (cancelSt cf state δ) with "[Hstate Hio Hcf]" as "HcSt".
      { iExists i. iFrame. iRight. by iFrame. }
      iApply (ewp_next with "[HpInv HpSt] [HcInv HcSt]").
      { iApply "HpInv". iApply "HpSt". }
      { iApply "HcInv". iApply "HcSt". } 
      by iApply "Hq".
      (* Cancel *)
      + iDestruct "Hcancel'" as "(%δ' & Hcmem')".
        ...
        * (* it was not active *)
          ...
          iApply (ewp_load with "Hstate"). iIntros "!> Hstate !>".
          iApply ("Hk" with "Hio").
          iApply ("IH_handler" with "Hγ Hq HpInv Hmem [HcInv Hstate Hcf] Hcmem Hnone").
          iApply "HcInv". iExists i. iFrame. iRight.
...
Section closed_proof.
  ...
  Local Program Instance async_comp_lib `{!promiseGS Σ} `{!cancelGS Σ}:
    ...
    io_log := λ v, (∃ (i : nat) δ, ⌜ v = #i ⌝ ∗ io_log_cancelled δ i)%I;
    io_log_Persistent := _;
  }.
  Next Obligation. 
    iIntros (????) "[#Hnone He]"; 
  ...
  Next Obligation.
    ...
    by iApply ewp_fiber_cancel.
    iIntros (v) "(%δ & %i & -> & Hio & Hc)".
    iModIntro. iExists i, δ. by iSplit.
  Qed.
