...
Definition queue_change_state : val := (λ: <>, #())%V.
Definition queue_create : val := (λ: "_", ref NONEV)%V.
Definition queue_push : val := (rec: "push" "s" "v" :=
    let: "tail" := Load "s" in
    let: "new" := SOME (ref ("v", "tail")) in
    if: CAS "s" "tail" "new" then #() else "push" "s" "v")%V.
Definition queue_pop : val := (rec: "pop" "s" :=
    match: Load "s" with
      NONE => NONEV
    | SOME "l" =>
      let: "pair" := Load "l" in
...
Section stacks.
  ...
  Definition is_queue (P : valO -d> iPropO Σ) v :=
    inv N (stack_inv P v).
  Definition is_queue_reader (P : valO -d> iProp Σ) v :=
    (is_queue P v ∗ deferred_mode_token mode_name)%I.
  ...
  Lemma is_queue_reader_is_queue P v:
    ⊢ is_queue_reader P v -∗ is_queue P v ∗ is_queue_reader P v.
  Proof.
    iIntros "(#Hq & Hmtok)". iSplit; first done. by iFrame.
  Qed.
  ...
  Global Instance is_queue_ne q n :
    Proper ((dist n) ==> (dist n)) (λ P, is_queue P q).
  Proof.
    intros P P' HPeq.
    rewrite /is_queue.
    (* f_contractive. *)
  ...
  Global Instance is_queue_reader_ne q n :
    Proper ((dist n) ==> (dist n)) (λ P, is_queue_reader P q).
  Proof.
    intros P P' HPeq.
    rewrite /is_queue_reader.
    f_equiv.
    by apply is_queue_ne.
  Qed.

  Global Instance is_queue_Persistent q P :
    Persistent (is_queue P q).
...
Section proofs.
  ...
  Theorem queue_register_push_Q (Q : iProp Σ) P s :
  ⊢ is_queue_reader N P s -∗
    EWP queue_change_state #() {{ _, 
      fulfill_permission Q ∗
      ∃ γ δ, push_permission γ δ Q }}.
  Proof.
    iIntros "(Hstack & Hmtok)". 
    (* rewrite fupd_ewp. *)
    rewrite /is_queue /stack_inv.
    iInv "Hstack" as (ℓ v') "[Heq [Hl [Hmode|Hmode]]]" "Hclose".
    2: {
      iDestruct "Hmode" as (Q') "(_ & Hmtok' & _)".
      unfold queue_change_state.
      ewp_pure_steps.
  ...
  Theorem queue_push_spec γ δ (Q : iProp Σ) P s v :
  ⊢ is_queue N P s -∗ 
    push_permission γ δ Q -∗ 
    ▷ (Q -∗ P v) -∗
    EWP queue_push s v {{ _, True }}.
  Proof.
    iIntros "#Hstack (#HQ & #Hin) HQP".
    iLöb as "IH".
    rewrite /queue_push.
    ewp_pure_steps.
    ewp_bind_rule; simpl.
    rewrite /is_queue /stack_inv.
    iApply (ewp_atomic ⊤ (⊤ ∖ ↑N)).
    iMod (inv_acc with "Hstack") as "((%ℓ & %v' & >-> & Hl & Hlist) & Hclose)"; first by done.
    iModIntro.
    ...
    ewp_pure_steps.
    iApply (ewp_bind [AppRCtx _; InjRCtx]); simpl.
    instantiate (1:=(AllocN (LitV 1%Z) (Pair v (oloc_to_val v')))).
    instantiate (1:=(Rec BAnon "new"
        (If (Snd (CmpXchg (LitV ℓ) (oloc_to_val v') "new")) 
           (LitV LitUnit)
           (RecV "push" "s"
              (Rec BAnon "v"
                 (Rec BAnon "tail"
                    (Rec BAnon "new"
                       (If (Snd (CmpXchg "s" "tail" "new")) 
                          (LitV LitUnit) ("push" "s" "v"))
                       (InjR (AllocN (LitV 1%Z) (Pair "v" "tail"))))
                    (Load "s"))) (LitV ℓ) v)))).
    cbn. reflexivity.
    (* a.d. TODO cannot evaluate under the AllocN *)
    iApply (ewp_bind' (AllocNRCtx (LitV 1%Z))); [cbn; reflexivity|simpl].
    ewp_pure_steps.
    iApply ewp_alloc.
    iIntros "!> %ℓ' Hl' !>".
    ewp_pure_steps.
    ...
    - iApply (ewp_cmpxchg_suc with "Hl").
      reflexivity. { destruct v''; left; done. }
      iIntros "!> Hl !>".
    ...
    - iApply (ewp_cmpxchg_fail with "Hl").
  ...
  Theorem queue_fulfill_Q (Q : iProp Σ) P s :
    ⊢ is_queue N P s -∗
      fulfill_permission Q -∗
      □ Q -∗
      EWP queue_change_state #() {{ _, is_queue_reader N P s }}.
  Proof.
    iIntros "#Hstack (Hmtok & Hfulfill) #Hq".
    rewrite /is_queue /stack_inv.
    iApply (ewp_atomic ⊤ (⊤ ∖ ↑N)).
    iMod (inv_acc with "Hstack") as "((%ℓ & %v' & >-> & Hl & [Hmode|(%Q' & Hmode)]) & Hclose)"; first by done.
    {
      iModIntro.
      iDestruct "Hmode" as "(_ & Hmtok' & _)".
      rewrite /queue_change_state. ewp_pure_steps.
  ...
  Theorem queue_pop_spec P s :
    ⊢ is_queue_reader N P s -∗
      EWP queue_pop s {{ ov, is_queue_reader N P s ∗ (⌜ov = NONEV⌝ ∨ ∃ v, ⌜ov = SOMEV v⌝ ∗ P v) }}.
  Proof.
    iIntros "(#Hstack & Hmtok)".
    iLöb as "IH".
    rewrite /queue_pop.
    ...
    - cbn. ewp_pure_steps. 
      ewp_bind_rule; simpl.
      iApply (ewp_atomic ⊤ (⊤ ∖ ↑N)).
      iMod (inv_acc with "Hstack") as "((%ℓ' & %v' & >-> & Hl' & [Hlist|Hlist]) & Hclose)"; first by done.
      iModIntro.
      2: {
      ...
      * iDestruct "Hlist" as "(Hlist & HRest)".
        ...
        iSplit; first done.
        by iFrame.
      * iApply (ewp_cmpxchg_fail with "Hl'"). { destruct v''; simpl; congruence. }
        now right.
        iIntros "!> Hl' !>".
        iMod ("Hclose" with "[Hl' Hlist]") as "_".
...
(* a.d. make sure we can instanitate the thing. *)
  ...
  Lemma normal_mode_init :
    ⊢ |==> ∃ _ : deferredGS Σ, ∀ P, normal_mode P None ∗ deferred_mode_token mode_name.
  ...
  Theorem queue_create_spec :
    ⊢ EWP queue_create #() {{ s, 
        ∃ _ : deferredGS Σ, ∀ P, |={⊤}=> (is_queue N P s ∗ is_queue_reader N P s) }}.
  Proof using deferredGpreS0 heapGS0 savedPropG0 Σ.
    rewrite /queue_create.
    ewp_pure_steps.
    iApply ewp_alloc.
    iIntros "!> %ℓ Hl".
    iMod (normal_mode_init) as (?) "H".
    iModIntro.
    iExists H.
    iIntros "%P".
    iDestruct ("H" $! P) as "(Hmode & Hmtok)".
