...
Proof.
  ...
  apply (Ectx_prim_step _ _ _ _ [] (fill_frame f (Eff m v k))
                                        (Eff m v (f :: k))); eauto.
...
Proof.
  ...
  inversion Hstep. simplify_eq. exists [], (fill (k ++ k0) e2'), σ', [].
  apply (Ectx_prim_step _ _ _ _ (k ++ k0) e1' e2'); eauto.
  by rewrite fill_app.
Qed.
...
Proof.
  ...
  intros ?. apply (Ectx_prim_step _ _ _ _ [] e1 e2); by eauto.
Qed.
...
Proof.
  ...
  - unfold heap_upd in H9. simpl in H9.
...
Lemma eff_not_pure m v k e : ¬ pure_prim_step (Eff m v k) e.
Proof.
  intros H0.
...
Lemma eff_not_pure' p e e' : to_eff e = Some p → ¬ pure_prim_step e e'.
Proof.
...
Lemma pure_prim_step_fill_frame f e e' :
  pure_prim_step e e' →
    pure_prim_step (fill_frame f e) (fill_frame f e').
Proof.
  constructor.
  ...
  - intros ??? Hstep.
    ...
    { by apply (reducible_not_eff _ σ1),
               (pure_prim_step_imp_reducible _ e'). }
    destruct (Ectxi_prim_step_inv f e _ _ _ not_val not_eff Hstep) as [e'' [He'' ->]].
    by destruct (pure_prim_step_det _ _ H _ _ _ He'') as [-> ->].
Qed.

Lemma pure_prim_step_fill k e e' :
  pure_prim_step e e' →
    pure_prim_step (fill k e) (fill k e').
Proof.
...
Lemma tc_pure_prim_step_fill k e e' :
  tc pure_prim_step e e' →
    tc pure_prim_step (fill k e) (fill k e').
Proof.
  induction 1.
  - apply tc_once.
...
Lemma rtc_pure_prim_step_fill k e e' :
  rtc pure_prim_step e e' →
    rtc pure_prim_step (fill k e) (fill k e').
Proof.
  induction 1; [done|].
...
Lemma tc_pure_prim_step_fill_frame f e e' :
  tc pure_prim_step e e' →
    tc pure_prim_step (fill_frame f e) (fill_frame f e').
Proof. by apply (tc_pure_prim_step_fill [f]). Qed.

Lemma rtc_pure_prim_step_fill_frame f e e' :
  rtc pure_prim_step e e' →
    rtc pure_prim_step (fill_frame f e) (fill_frame f e').
Proof. by apply (rtc_pure_prim_step_fill [f]). Qed.

Lemma rtc_pure_prim_step_Eff `{NeutralEctx k} m v k' :
  rtc pure_prim_step (fill k (Eff m v k')) (Eff m v (k ++ k')).
Proof.
  induction k as [|f k].
  - done.
  - specialize (ConsCtx_neutral_inv' _ _ H) as f_neutral.
    ...
    + by apply rtc_pure_prim_step_fill_frame; auto.
