...
(* This file formalizes the small-step operational semantics of [eff_lang] as
   ...
   (2) The construct [ContV k l] reduces to [fill k v] (the "filling of the
       ...
       we have the following rules:
              ...
              (ContV k l, σ[l ↦ false]) --> (fill k v, σ[l ↦ true])
                          (KontV k, σ]) --> (fill k v, σ)
...
Inductive prim_step (e1 : expr) (σ1 : state) (e2 : expr) (σ2 : state) : Prop :=
...
Lemma Ectxi_prim_step f e1' e2' e1 σ1 e2 σ2 :
  e1 = fill_frame f e1' →
    e2 = fill_frame f e2' →
      head_step e1' σ1 e2' σ2 →
        prim_step e1  σ1 e2  σ2.
Proof.
  intros -> -> H.
  by apply (Ectx_prim_step _ _ _ _ [f] e1' e2').
Qed.
...
Proof.
  ...
  by apply (Ectx_prim_step _ _ _ _ (f :: k) e1'0 e2'0).
Qed.
...
Proof.
  intros -> ->. inversion 1. simplify_eq.
  apply (Ectx_prim_step _ _ _ _ (k ++ k0) e1'0 e2'0); eauto;
  by rewrite fill_app.
Qed.
...
Lemma Ectxi_prim_step_inv f e e₂ σ₁ σ₂ :
  to_val e = None →
    to_eff e = None →
      prim_step (fill_frame f e) σ₁ e₂ σ₂ →
        ∃ e', prim_step e σ₁ e' σ₂ ∧ e₂ = fill_frame f e'.
Proof.
  intros ?? Hstep.
...
Proof.
  ...
  - simpl. intros Hstep.
    destruct (Ectxi_prim_step_inv f _ _ _ _ (fill_not_val k _ H)
